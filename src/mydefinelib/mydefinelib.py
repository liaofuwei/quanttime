...
def get_appoint_trade_date(strInputDate, nDays):
    ...
    '''
    if "/" in strInputDate:
        input_date = datetime.strptime(strInputDate,"%Y/%m/%d").strftime("%Y-%m-%d")
    input_date = strInputDate

    if nDays==0:
        return input_date

    df_trade_date = pd.read_csv("C:\\quanttime\\data\\basic_info\\all_trade_day.csv", index_col=["trade_date"])
...
def get_trade_date_by_year_month(nYear, nMonth):
    ...
    index_st = df_trade_date.index.tolist().index(trade_date_st)
